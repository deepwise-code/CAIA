...
class Task(object):
    ...
    def sync_bn_runing_mean_var(self):
        """在评测前, 同步BN的运行时均值/方差"""
        for name, module in self.net.named_modules():
            if isinstance(module, nn.modules.batchnorm._BatchNorm):
                ...
                # import ipdb
                # ipdb.set_trace()
                # module.running_mean = module.module[0].running_mean.clone().detach()
                # module.running_var = module.module[0].running_var.clone().detach()
                # 取均值
                # module.running_mean = torch.mean(module.running_mean)
                # module.running_var = torch.mean(module.running_var)
