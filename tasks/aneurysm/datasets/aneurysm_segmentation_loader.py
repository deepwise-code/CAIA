...
def read_data(subject):
    ...
    if os.path.exists(aneurysm_path):
        aneurysm = np.asarray(nib.load(aneurysm_path).dataobj)
    else:
...
def sampe_subjects(subjects, aneurysm_bbox, patch_num):
    ...
    for subject in subjects:
        try:
            ...
            img, gt, _delayed_transform_add_coords = AneurysmSampler(\
                subject,  aneurysm_bbox[subject], spacing_zxy, image, aneurysm).sample(patch_num)
        except Exception as e:
            print('[warning] async load data with exception: {} {}'.format(subject, e))
            continue
        img_lst.append(img)
        gt_lst.append(gt)
